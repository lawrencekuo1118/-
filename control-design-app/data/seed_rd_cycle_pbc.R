#!/usr/bin/env Rscript
# Seed 研發循環 PBC 清單＋規格說明 → pbc_registry
# Usage: Rscript data/seed_rd_cycle_pbc.R
...
raw_path <- file.path(root, "data", "rd_cycle_pbc_seed_raw.json")
...
if (nrow(existing)) {
  drop <- existing$source == "rd_cycle_pbc_seed" |
    grepl("^PBC-RD-", existing$pbc_id)
...
n_rd <- sum(existing$cycle == cycle_name)
message(sprintf(
  "Seeded RD-cycle PBC: %d rows (RD total %d, registry %d, linked %d) → %s",
  nrow(reg), n_rd, nrow(existing), n_linked, out_csv
))
