#!/usr/bin/env Rscript
# Seed 銷售及收款循環（銷售收入）PBC 清單＋規格說明 → pbc_registry
# Usage: Rscript data/seed_sc_cycle_pbc.R
...
raw_path <- file.path(root, "data", "sc_cycle_pbc_seed_raw.json")
...
n_sc <- sum(existing$cycle == cycle_name)
message(sprintf(
  "Seeded SC-cycle PBC: %d rows (SC total %d, registry %d, linked %d) → %s",
  nrow(reg), n_sc, nrow(existing), n_linked, out_csv
))
